# ETP Security Fix #1: Entanglement Key Shard ID Exposure

**Date:** 2026-02-24  
**Issue:** CRITICAL — Entanglement key leaks shard IDs, enabling unauthorized reconstruction  
...
The problem is actually **deeper than the entanglement key itself**. There are THREE
information leakage points that form a kill chain:

```
CURRENT DESIGN — ATTACK CHAIN:

    ┌─ LEAK 1: Entanglement Key (plaintext, in transit)
...
**Key insight:** Fixing only the entanglement key isn't enough. If shards are stored in
plaintext and the commitment log is public, an attacker who knows the entity_id can
reconstruct the entity without ever seeing the entanglement key.
...
**What changes:** Encrypt the entire entanglement key to the receiver's public key using
...
MATERIALIZE PHASE:
  1. Decrypt entanglement key → get entity_id, CEK, shard_ids
...
**What changes:** Everything from Option B, PLUS: remove shard_ids from the entanglement
...
So the entanglement key shrinks to its **theoretical minimum**:

```
ENTANGLEMENT KEY (new):
  SealedBox(receiver_pubkey, {
...
| Pros | Cons |
|------|------|
| Entanglement key shrinks from ~869B to ~160B | Nodes must support fetch-by-(entity_id, index) not just by shard_id |
...
token before serving shards. The sender mints tokens at entangle time.
...
2. Shrinks the entanglement key from ~869 bytes to ~160 bytes (closer to the "O(1) key" claim)
...
attacker can unseal the entanglement key and obtain the CEK. This is fundamental — it's the
